function E=strain2D(Ux,Uy,filter_columns)
...
%if(~exist('Uz','var')) % Detect if 2D or 3D inputs
    ...
    % [Uxy,Uxx] = gradient(Ux);% [changes in horizontal direction, changes in vertical direction]=gradient();
    % [Uyy,Uyx] = gradient(Uy);
    % input: Ux vertical, Uy horizontal
    
     [Uxy,Uxx] = xd_gradient2D_smooth(Ux, filter_columns);    
     [Uyy,Uyx] = xd_gradient2D_smooth(Uy, filter_columns);
