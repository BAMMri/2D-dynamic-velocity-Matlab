...
% %Adapt variables
venc = 10;%cm/s sAngio.sFlowArray.asElm[0].nVelocity
thres=1;
scale_factor =16;step=1;  %default 8
tDelay =5;%trigger delay sPhysioImaging.sPhysioExt.lTriggerDelay/1000
%AcqTime=input('Acq Time: ');%4000 1 % 
AcqTime=3500;
...
limit=29;%001_FL_PC_3DIR number of letters from 0001_ on, usually 13; 29 for venc10; 35 for venc10_right
...
num1=  str2double( filename_mx((size(filename_mx,2)-(limit+3))));%Claudia: check for scannumbers>100
if num0>0
    num=num+num0*10+num1*100;
end
if ((num+2)<10)
...
if (num1>0)%if scannumber is>=100, Claudia
    filename_fx = [head_folder '\' num2str(num+2) namef '_P'];
    filename_fy = [head_folder '\' num2str(num+4) namef '_P'];
    filename_fz = [head_folder '\' num2str(num+6) namef '_P'];
end    
drawnow

[mag_x, info_mx] = load3dDicom(filename_mx);
[flow_x0, info_fx] = load3dDicom(filename_fx);
[flow_y0, info_fy] = load3dDicom(filename_fy);
[flow_z0, info_fz] = load3dDicom(filename_fz);
...
dt= round(10*(info_fx(1,2).RepetitionTime)/interp_factor)*0.1;
...
for iP=1:size(flow_x,3)
    %f3D(:,:,iP)=f3D(:,:,iP).*(f3D(:,:,iP)>0.25);   %get rid of low values
